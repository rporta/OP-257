...
class MoviPlus
{
    ...
    public function __construct($presubscription)
    {
		...
		$sponsorCode = $this->config->get('sponsor.' . $presubscription->SponsorId);

		$url = str_replace('{click_id}', $presubscription->Pixel, $this->config->get('adNetworks.79.url'));
		if (isset($sponsorCode)){
			$url = str_replace('{sponsor_code}', $sponsorCode, $url);
		}
